...
def compute_and_save(output_exp_dir, output_data_sample_name, query, rg, data_exp_name, data_sample_name, logger, explainer_configs):    
    metrics_npz_file = os.path.join(output_exp_dir, 'scores', f'{output_data_sample_name}_anchor_metrics_{rg[0]}_{rg[1]}.npz')
    ...
    sst = time.time()
    th = explainer_configs.get('threshold') if explainer_configs.get('threshold') != None else 0.9999
    tau = explainer_configs.get('tau') if explainer_configs.get('tau') != None else 0.2
    logger.info(f'[{get_time_str()}] start computing anchor from index: {previous_work_idx} to {end - 1} with config: {th} {tau}')
    count = 0
    for i in range(previous_work_idx, end):
        exp = explainer.explain_instance(paper_data[i], pred_fn, threshold=th, tau=tau)
...
within ({ett} total / {avg_time} average)')
    ...
    end_record_paper_count(task_name)
    logger.info(f'[{get_time_str()}] end anchor metrics witin {round(time.time() - st, 6)}')
...
def get_anchor_metrics(exp_dir_path, query, task_config, explainer_configs, data_info):
    ...
    if not os.path.exists(log_dir):
        os.mkdir(log_dir)
    log_file_path = os.path.join(log_dir, f'{current_sample_name}_anchor_{rg}.log')
