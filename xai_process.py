...
class XaiProcess:
    ...
    def __init__(self, exp_name, sample_name) -> None:
        ...
        if samples_from_other_exp_configs != None and sample_name in samples_from_other_exp_configs.keys():
            other_exp_name, data_file_name = samples_from_other_exp_configs.get(sample_name)
            self.data_info = {
    ...
    def ale(self, is_2w = False):
        sample_config = self.get_sample_config_for_this_method('ale_2w' if is_2w else 'ale_1w')
        s2search_score_ale.get_ale_and_save_score(self.exp_dir_path, is_2w, sample_config, self.data_info)
        plotting_nb_gen.gen_for_ale(self.exp_name, self.description, [self.sample_name])
    ...
    def check_query(self):
        ...
        paper_data = getting_data.load_sample(self.data_info['sample_src_exp_name'], self.data_info['sample_src_name'], not_df=True)
        paper_data_len = len(paper_data)
        
        for query in sample_config:
            ranker_helper.start_record_paper_count(f'check query {self.exp_name} {self.sample_name}')
